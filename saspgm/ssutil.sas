options source;
%macro ssutil(__func__,   ___mn___ =, ___mv___ =,  ___mt___ =,
                ...
                / des='SS macro SSUTIL 1.0';

%if %upcase(&__func__)=HELP %then %do;
  %put;
  %put %str( ------------------------------------------------------------------------------);
  %put %str( HELP: SS macro SS2UTIL                                                      );
  %put %str( ------------------------------------------------------------------------------);
  %put %str( A SS utility macro: Check existance of datasets, variables, values etc. The);
...
%if (%length(&__dsn__) = 0 ) and
  ...
  %put Error: Please specify dataset SSUTIL(__DSN__=). &__amsg__;
  %let __err__=ERROR; %let _reason_=SYNTAX;
  %goto exit;
%end;
